...
def find_best_routes_by_train(route_by_train, railroad):
...
def find_connected_cities(board, railroad, cell, dist):
    tiles = itertools.chain.from_iterable(walk_routes(board, railroad, None, cell, dist))
    return {tile.cell for tile in tiles if tile.is_city} - {cell}

def walk_routes(board, railroad, enter_from, cell, length, visited=None):
    visited = visited or []
    ...
    for neighbor in neighbors:
        neighbor_paths = walk_routes(board, railroad, cell, neighbor, remaining_cities, visited + [tile])
...
def find_routes_from_cell(board, railroad, cell, train):
    tile = board.get_space(cell)
    if not tile.is_city:
        raise Exception("How is your station not in a city? {}".format(cell))

    return walk_routes(board, railroad, None, cell, train.visit)

def find_all_routes(board, railroad):
    ...
    for train in railroad.trains:
        ...
        for station in stations:
            routes[train].update(find_routes_from_cell(board, railroad, station.cell, train))

            connected_cities = find_connected_cities(board, railroad, station.cell, train.visit - 1)
            connected_paths = set()
            for cell in connected_cities:
                for path in find_routes_from_cell(board, railroad, cell, train):
...
def find_best_routes(board, railroads, active_railroad):
    routes = find_all_routes(board, active_railroad)
    ...
    return find_best_routes_by_train(route_value_by_train, active_railroad)
